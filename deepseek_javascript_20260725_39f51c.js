...
// ---------- Embedded frontend HTML (no public folder needed) ----------
...
async function resolveSlug(query) {
  ...
  try {
    ...
    if (first.length) {
      ...
      if (m) {
        console.log(`🔗 Found slug via search: ${m[1]}`);
        return m[1];
      }
    }
  } catch (e) { console.warn('Search fallback failed:', e.message); }
  throw new Error('Could not resolve manga. Please provide the full comix.to URL or slug.');
}
...
async function getMangaData(slug) {
  ...
  if (!chapters.length) {
    ...
    const chapterSelectors = [
      '.chapter-list a', '.chapters a', '.wp-manga-chapter a', '.chapter-item a',
      '.series-chapters a', '.chapter-link', 'a[href*="/chapter/"]', 'a[href*="chapter-"]',
      '.list-chapter a', '.chapter-container a', '.chapters-list a', '.episode-list a',
      '.volume-list a[href*="chapter"]', '.manga-chapters a', '.chapter a'
    ];

    for (const sel of chapterSelectors) {
      const els = $(sel);
      if (els.length) {
        console.log(`🔍 Found ${els.length} links with selector: ${sel}`);
        els.each((i, el) => {
          const link = $(el);
    ...
    if (!chapters.length) {
      console.warn('⚠️ No chapters found with selectors, scanning all links...');
      $('a').each((i, el) => {
        const link = $(el);
        const href = link.attr('href');
...
async function getChapterImages(chapterUrl) {
  console.log(`🖼️ Fetching images from: ${chapterUrl}`);
  ...
  if (!images.length) {
    const selectors = [
      '.chapter-images img', '.reader-area img', '.page-img img', '.reading-content img',
      '.chapter-content img', '.comic-page img', '.page-image img', '.image-container img',
      '.chapter-body img', '.main-content img', '.reader img', '.viewer img',
      '.comic-reader img', '.content-area img', '.manga-page img', '.page img'
    ];
  ...
  if (!images.length) {
    console.warn('⚠️ Still no images, scanning all images...');
    $('img').each((i, el) => {
...
async function createCBZStream(imageUrls) {
  ...
  for (const url of imageUrls) {
    try {
      console.log(`⬇️ Downloading image ${idx}/${imageUrls.length}: ${url}`);
...
app.get('/api/download', async (req, res) => {
  ...
  console.log(`⬇️ Download request for ch.${chapter} (${url})`);
  try {
...
app.post('/api/download-multiple', async (req, res) => {
  ...
  console.log(`📦 Multi-download for ${chapters.length} chapters`);
  try {
    const archive = archiver('zip', { zlib: { level: 9 } });
    const pass = new stream.PassThrough();
    archive.pipe(pass);
    let total = 0;
    for (const ch of chapters) {
      console.log(`⬇️ Processing ch.${ch.num} (${ch.url})`);
